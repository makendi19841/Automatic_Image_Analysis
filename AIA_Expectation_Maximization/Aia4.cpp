...
Mat Aia5::calcCompLogL(const vector<struct comp*>& model, const Mat& features){
	...
	int dimensionNumber = features.rows;

	// Number of samples
	int featuresNumber = features.cols;

	// Number of components
	int componentNumber = model.size();

	Mat logLikehood(featuresNumber, componentNumber, CV_32FC1);
	...
	for (int i = 0; i < featuresNumber; i++) {
		for (int j = 0; j < componentNumber; j++) {
			...
			logLikehood.at<float>(i, j) = -(dimensionNumber / 2.0f) * log(2 * CV_PI) - 0.5f*log(determinant(covar)) - 0.5f*((Mat)(diff.t()*covar.inv()*diff)).at<float>(0, 0);
		}
	}
...
Mat Aia5::calcMixtureLogL(const vector<struct comp*>& model, const Mat& features){
	...
	// combine likelihood in all model components
	Mat combinelogLikelihood = calcCompLogL(model, features);

	Mat logLikehood(featuresNumber, 1, CV_32FC1);

	for (int i = 0; i < featuresNumber; i++) {

		//  scaling factor c
		double logC = log(model.at(0)->weight) + combinelogLikelihood.at<float>(i, 0);
		for (int j = 1; j < model.size(); j++) {
			logC = logC < log(model.at(j)->weight) + combinelogLikelihood.at<float>(i, j) ? log(model.at(j)->weight) + combinelogLikelihood.at<float>(i, j) : logC;
		}

		//  sum
		double sum = 0;
		for (int j = 0; j < model.size(); j++) {
			sum += exp(log(model.at(j)->weight) + combinelogLikelihood.at<float>(i, j) - logC);
		}

		logLikehood.at<float>(i) = logC + log(sum);
	}
...
Mat Aia5::gmmEStep(const vector<struct comp*>& model, const Mat& features){
	...
	Mat mixturelogLikelihood = calcMixtureLogL(model, features);

	for (int i = 0; i < numberOfFeatures; i++) {
		for (int j = 0; j < numberOfComponents; j++) {
			posterior.at<float>(i, j) = exp(log(model.at(j)->weight) + combinelogLikelihood.at<float>(i, j) - mixturelogLikelihood.at<float>(i));
		}
	}
...
void Aia5::gmmMStep(vector<struct comp*>& model, const Mat& features, const Mat& posterior){
   ...
   // parameter update 
	...
	for (int i = 0; i < model.size(); i++) {
		...
		// initialization
		double n_ = 0;
		Mat mean_ = Mat::zeros(features.rows, 1, CV_32FC1);
		Mat sigma_ = Mat::zeros(features.rows, features.rows, CV_32FC1);

		// update the mean
		for (int j = 0; j < features.cols; j++) {
			n_ += posterior.at<float>(j, i);
			mean_ += features.col(j)*posterior.at<float>(j, i);
		}

		mean_ /= n_;

		// update the covariance
		Mat diff = features.col(0) - mean_;
		sigma_ = (diff*diff.t())*posterior.at<float>(0, i);
		for (int j = 1; j < features.cols; j++) {
			diff = features.col(j) - mean_;
			sigma_ += (diff*diff.t())*posterior.at<float>(j, i);
		}

		sigma_ /= n_;

		model.at(i)->mean = mean_;
		model.at(i)->covar = sigma_;
		model.at(i)->weight = n_ / features.cols;
	}
}
...
void Aia5::readImageDatabase(string dataPath, vector<Mat>& db){
	...
	list<Mat> imgList;
	sprintf_s(curDir, "%s%c%i%c", dataPath.c_str(), delim, c, delim);
